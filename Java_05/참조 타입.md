...
기본 타입과, 참조 타입으로 선언된 변수의 차이점은 저장되는 값이 무엇이냐 인데, 기본 타입으로 선언된 변수는 실제 값을 변수 안에 저장하나, 참조 타입인 배열, 열거 클래스, 인터페이스를 이용해 선언된 변수는 메로리의 번지를 값으로 갖는다. 번지를 통해 객체를 참조한다는 뜻에서 참조 타입이라고 부른다.
...
열거 타입의 모든 열거 객체들을 배열로 만들러 리턴한다. 다음은 Week 열거 타입의 모든 열거 객체를 배열로 만들어 향상된 for문을 이용해 반복하는 코드이다.
